...
// Set up trust proxy for accurate IP identification behind a reverse proxy (like Vercel / Cloud Run)
...
// Models tried in order of speed. Flash is the fastest; the others are fallbacks.
const MODELS_TO_TRY = ["gemini-flash-latest", "gemini-3.6-flash", "gemini-3.1-pro-preview"];

// Keep the last N conversation turns to bound input latency.
const MAX_CONVERSATION_TURNS = 12;

// AI Assistant Chat endpoint. Streams plain-text chunks for fast first-token latency.
app.post("/api/chat", chatLimiter, async (req, res) => {
  ...
  // Stream-friendly response headers
  ...
  if (!aiClient) {
    ...
    res.flushHeaders?.();
    res.write(getOfflineKnowledgeResponse(lastUserMsg));
    return res.end();
  }

  try {
    // Prepare multi-turn conversation history for Gemini API (last MAX_CONVERSATION_TURNS only)
...
- EASTER EGG: If the user says/types "automate me" or requests an "automation roast", deliver a fun, witty, sci-fi-themed "automation roast" targeting manual spreadsheet copying, manual CRM entry, or repetitive copy-pasting (without using * or #)!`;
    ...
    // Stream from the fastest working model, falling back if nothing is produced.
    res.flushHeaders?.();
    let committed = false;
    ...
    // If all models failed before producing any content, use offline knowledge base engine
    if (!committed) {
      console.warn("All Gemini models exhausted. Falling back to offline knowledge engine.", lastError);
      res.write(getOfflineKnowledgeResponse(lastUserMsg));
    }
