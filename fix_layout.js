...
code = beforeFormEnd + '              </div>\n\n              {/* Right Column: Booking Widget */}\n              <div className="h-full">\n' + calendlyWidget.split('\n').map(l => '  ' + l).join('\n') + '\n              </div>\n' + afterFormEnd;

// Also change height from 600/550 to 700 to match Calendly script
code = code.replace('className="w-full h-[600px] rounded-2xl"', 'className="w-full h-[700px] rounded-2xl"');
code = code.replace('height="600"', 'height="700"');
code = code.replace('min-h-[550px]', 'min-h-[700px]');
...
code = code.replace('w-full rounded-2xl overflow-hidden border border-white/10 bg-slate-950/80 min-h-[700px] shadow-inner', 'w-full flex-grow rounded-2xl overflow-hidden border border-white/10 bg-slate-950/80 min-h-[700px] shadow-inner flex flex-col');
